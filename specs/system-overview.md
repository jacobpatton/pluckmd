...
harvest downloads article listings into Markdown files. It targets blogs,
...
Provides the `harvest` executable and implements:
...
1. User runs `harvest download <url>`.
...
2. User opens a page in Chrome and runs `harvest download --active-tab`.
...
1. User runs `harvest inspect <url>` or `harvest inspect --active-tab`.
